...
const Header = () => {
    ...
    if (allRequests.results) {
        ...
        requestsSent = requests.filter(element => element.requester.id === localStorage.id);            
        requestsReceived = requests.filter(element => element.requester.id !== localStorage.id);
    }
            ...
            <NavBar>
                ...
                <Right>
                    ...
                    <button className="user" onClick={menu}>
                        <img src={localStorage.profilePic ? localStorage.profilePic : jennifer} alt="user icon"/>
                    </button>
                ...
                <NotificationDropDown style={{display: notificationsClick ? "flex" : "none"}}>
                    <div>
                        ...
                        { requests.length > 0 ? requestsReceived.length > 0 ? requestsReceived.map(element => <NotificationComponent key={element.id} request={element}/>) : "No requests" : "No requests" }
                    </div>
                    <div style={{marginTop: "10px"}}>
                        <p>Sent requests</p>
                        { requests.length > 0 ? requestsSent.length > 0 ? requestsSent.map(element => <NotificationComponent key={element.id} request={element}/>) : "No requests" : "No requests"}                    
                    </div>
